 ...
 */
export function formatFlowRateToMonthly(flowRate: bigint, decimals: number = 18): string {
 ...
 */
export function normalizeTxHash(hash: unknown): `0x${string}` {
  if (typeof hash === "string") {
    const cleanHash = hash.trim().startsWith("0x") ? hash.trim() : `0x${hash.trim()}`;
    const hexPart = cleanHash.slice(2);
    return `0x${hexPart.length % 2 === 0 ? hexPart : `0${hexPart}`}` as `0x${string}`;
  }
  return `0x${String(hash).padStart(64, "0")}` as `0x${string}`;
}
