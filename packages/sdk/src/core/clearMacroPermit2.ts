import { hashStruct, type Address, type Hex, type PublicClient } from "viem";
...
export interface BuildPermit2MacroStreamParams {
  ...
  walletClient: { signTypedData: (args: Record<string, unknown>) => Promise<Hex> };
  account: Address;
...
export async function buildPermit2MacroStreamExecution(
  params: BuildPermit2MacroStreamParams,
): Promise<Permit2MacroExecution> {
  ...
  if (!forwarder) throw new Error("ClearMacro: no forwarder address (config.clearMacro.forwarder)");
  if (!macro) {
    throw new Error(
      "ClearMacro: no CreateFlowMacro address. Set config.clearMacro.createFlowMacro or pass macroAddress.",
    );
  }
  ...
  const actionLeaves: Record<string, unknown> = {
    description,
    superToken,
    token: superToken,
    receiver: recipient,
  ...
  for (const f of types.Security) {
    securityMessage[f.name] = (security as Record<string, unknown>)[f.name];
  }
  ...
  if (witnessLocal.toLowerCase() !== witnessHash.toLowerCase()) {
    throw new Error(
      "ClearMacro: reconstructed Permit2 witness does not match getPermit2WitnessStructHash " +
...
export async function submitPermit2MacroExecution(
...
export async function createStreamViaPermit2Macro(
  params: CreateStreamViaPermit2MacroParams,
): Promise<{ execution: Permit2MacroExecution; txHash: Hex }> {
 ...
 */
export function facilitatorPermit2Relay(
  relayUrl: string,
  fetchImpl: typeof fetch = fetch,
): (execution: Permit2MacroExecution) => Promise<Hex> {
